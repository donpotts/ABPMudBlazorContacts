...
namespace ContactsProject.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("ContactsProject") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "ContactsProject", DisplayName = "ContactsProject API", Resources = { "ContactsProject" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["ContactsProject_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["ContactsProject_Web:RootUrl"]!.EnsureEndsWith('/');

            /* ContactsProject_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["ContactsProject_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["ContactsProject_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["ContactsProject_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // Console Client
        var consoleClientId = configurationSection["ContactsProject_App:ClientId"];
        if (!consoleClientId.IsNullOrWhiteSpace())
        {
                await CreateApplicationAsync(
                    ...
                    secret: configurationSection["ContactsProject_App:ClientSecret"] ?? "1q2w3e*",
